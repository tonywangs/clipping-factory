"""Local LLM backend — Anthropic, OpenAI, or Gemini, selected by LLM_PROVIDER."""
from ..config import (
    ANTHROPIC_MODEL,
    GEMINI_MODEL,
    LLM_PROVIDER,
...
def call_anthropic_llm(prompt: str) -> str:
    """Anthropic Messages backend used by --mode local."""
    try:
        from anthropic import Anthropic  # type: ignore
    except ImportError as e:
        raise RuntimeError("anthropic is required for LLM_PROVIDER=anthropic.") from e
    response = Anthropic(api_key=require_anthropic_key()).messages.create(
