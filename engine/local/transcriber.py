...
def _transcript_json_cache_path(media_path: str) -> Path:
    cache_dir = Path(LOCAL_OUTPUT_DIR)
    cache_dir.mkdir(parents=True, exist_ok=True)
...
def transcribe_local(media_path: str, language: Optional[str] = None) -> Dict:
    """Run faster-whisper with word timestamps, caching JSON and a compatible SRT."""
    json_cache = _transcript_json_cache_path(media_path)
    if json_cache.exists() and json_cache.stat().st_mtime >= os.path.getmtime(media_path):
        cached = json.loads(json_cache.read_text(encoding="utf-8"))
        if cached.get("segments"):
            print(f"[transcribe/local] reusing word-timestamp cache: {json_cache}", flush=True)
            return cached
    cache_path = _transcript_cache_path(media_path)
    ...
    for s in segments_iter:
        segments.append({
            ...
            "words": [
                {"word": (word.word or "").strip(), "start": float(word.start), "end": float(word.end)}
    ...
    json_cache.write_text(json.dumps(transcript, indent=2), encoding="utf-8")
