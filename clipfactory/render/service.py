...
from ..ffmpeg_bin import ffmpeg_bin, ffprobe_bin, require_caption_ffmpeg
...
def _audio_visual(source: Path, target: Path, duration: float, accent: str) -> None:
    ...
    graph = (
        f"color=c=0x{color}:s=1080x1920:d={duration}[bg];"
        f"[0:a]showwaves=s=920x350:mode=line:colors=white[wave];"
        f"[bg][wave]overlay=(W-w)/2:(H-h)/2,format=yuv420p[v]"
    )
    _run(
        [
            ffmpeg_bin(),
...
def _loudnorm_measure(source: Path) -> str | None:
    result = subprocess.run(
        [ffmpeg_bin(), "-i", str(source), "-af", "loudnorm=I=-14:TP=-1.5:LRA=11:print_format=json", "-f", "null", "-"],
        text=True,
        capture_output=True,
    )
    try:
        ...
        return (
            "loudnorm=I=-14:TP=-1.5:LRA=11:measured_I={input_i}:measured_LRA={input_lra}:"
            "measured_TP={input_tp}:measured_thresh={input_thresh}:offset={target_offset}:linear=true"
        ).format(**data)
...
def _burn_and_mix(source: Path, ass: Path, target: Path, niche: NicheConfig, duration: float, music: Path | None) -> None:
    ...
    if music:
        ...
        music_base = (
            f"[1:a]volume={niche.music_volume_db}dB,afade=t=in:d=0.5,"
            f"afade=t=out:st={max(0, duration - .5):.3f}:d=0.5[music]"
        )
        filter_candidates = [
            (
                f"{music_base};[0:a]asplit=2[speech][sc];[music][sc]sidechaincompress=threshold=0.02:ratio=8[ducked];"
                f"[speech][ducked]amix=inputs=2:duration=first:dropout_transition=0,{norm}[audio]"
            ),
