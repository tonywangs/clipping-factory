...
def write_ass(words: list[Word], start: float, end: float, style: CaptionStyle, accent: str, output: Path) -> Path:
    ...
    for group in _groups(visible):
        ...
        if style == CaptionStyle.KARAOKE:
            ...
            subs.events.append(
                pysubs2.SSAEvent(
                    start=int((group_start - start) * 1000),
                    end=int((group_end - start) * 1000),
                    text=" ".join(chunks),
                    style="Caption",
                )
            )
        ...
        for index, active in enumerate(group):
            ...
            text = " ".join(
                f"{{\\c{accent_ass}}}{_escape(word.word.upper())}{{\\c&HFFFFFF&}}" if word is active else _escape(word.word.upper())
                for word in group
            )
            subs.events.append(
                pysubs2.SSAEvent(
                    start=int((segment_start - start) * 1000),
                    end=int((segment_end - start) * 1000),
                    text=text,
                    style="Caption",
                )
            )
